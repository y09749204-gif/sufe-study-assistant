...
首次启动自动进入六步设置向导，支持保存课表草稿并继续上次进度。学校连接、AI 和首次同步均可跳过。详见 [首次设置指南](docs/GETTING_STARTED.md)。

**CLI / Personal OS 接入**：安装版随附 CLI，可查询课程、日历和待办，完成待办、触发同步并查询任务状态。详见 [CLI 调用文档](docs/CLI.md)。学业助手可单独使用，不依赖 Personal OS。
